function calculateTimer(timeInSeconds: number, timeInSecondsDescanso: number): Array<number|string>{
    ...
    let minutesFormatDescanso = minutes < 10 ? `0${minutes}` : minutes;
    let secondsFormatDescanso = seconds < 10 ? `0${seconds}` : seconds;

    return [
        minutesFormat,
        secondsFormat,
        secondsFormatDescanso,
        minutesFormatDescanso
    ];
}
